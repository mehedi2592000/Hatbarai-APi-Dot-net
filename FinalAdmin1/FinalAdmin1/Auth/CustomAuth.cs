...
namespace FinalAdmin1.Auth
{
    public class CustomAuth:AuthorizationFilterAttribute
    {
        public override void OnAuthorization(HttpActionContext actionContext)
        {
            ...
            if (authHeader == null)
            {
                actionContext.Response = actionContext.Request.CreateResponse(HttpStatusCode.NotFound, "sory");
            }
            else
            {
                string tok = authHeader.ToString();
                var re = AuthServices.CheckValidityToken(tok);
                if (!re)
                {
